...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 All occurrences of the detected absolute home directory path in git history are replaced with $HOME.
- [ ] #2 A case-insensitive scan of every commit for the audit detection terms returns zero matches.
- [ ] #3 Commits unaffected by the replacement retain their original hashes, and total commit count is unchanged.
- [ ] #4 A full backup bundle of all refs exists outside the repository before the rewrite.
- [ ] #5 origin/main is updated using --force-with-lease pinned to the expected pre-rewrite head SHA.
<!-- AC:END -->
